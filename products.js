...
try {
  if (window.supabase && typeof SUPABASE_URL !== "undefined" && typeof SUPABASE_ANON_KEY !== "undefined") {
    shopSupabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
  }
} catch (error) {
...
function addToCart(productId) {
  ...
  if (!product.available) {
    showToast("Sorry, this product is sold out");
    return;
  }
...
function renderCart() {
  ...
  if (!cart.length) {
    container.innerHTML = `<div style="padding:30px 15px;text-align:center;color:#777;"><div style="font-size:40px;">🛒</div><p>Your cart is empty.</p></div>`;
...
function showToast(message) {
  const old = document.getElementById("collins-toast");
  if (old) old.remove();
